...
import { GainChip, Pop, Reveal } from "./moment-ui";
...
export function EquipmentView({ state, navigate, onEquip, disabled }: {
  ...
  return <section className="collection-page">
    ...
    <div className="equipment-grid slots">{slots.map(([slot, label]) => {
      ...
      return <article className={`equipment-panel ${item ? "filled" : "empty"}`} key={slot} aria-label={label}>
        <div className="section-heading"><h2>{label}</h2>{slot === "weapon" ? <Sword size={20} /> : <Shield size={20} />}</div>
        {item ? <><Reveal id={item.key} className="equipped-detail"><img src={assetPath(item.asset_key)} alt="" width={48} height={48} /><div><strong>{item.name}</strong><p>+{number(item.combat_bonus)} chiến lực</p></div></Reveal><button className="secondary-button" disabled={disabled} onClick={() => onEquip(slot, null)} aria-label={`Tháo ${item.name}`}>Tháo trang bị</button></> : <div className="empty-slot">{slot === "weapon" ? <Sword size={26} /> : <Shield size={26} />}<p className="muted">Ô trống</p></div>}
